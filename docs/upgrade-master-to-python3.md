...
- Images are pinned: `yvess/desk-dns:0.5.0`, `yvess/desk-worker:0.5.0`. They
  are independent -- the dns image is not built on the worker image -- so a
  PowerDNS update rebuilds only `desk-dns` (see the PowerDNS runbook,
