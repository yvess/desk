...
def run(source, worker_type, *argv):
    """Run `source` in a subprocess and return (completed process, modules)."""
    wrapper = source + (
        "\nimport json, sys"
        "\nsys.stderr.write('MODULES ' + json.dumps(sorted(sys.modules)))"
        "\nsys.exit(globals().get('_rc', 0))\n"
    )
...
class DnsNodeImportsTestCase(unittest.TestCase):
    def test_a_worker_loads_none_of_the_foreman_stack(self):
        process, modules = run(DWORKER_MAIN, 'worker', '--help')
...
class RoleCommandsTestCase(unittest.TestCase):
    ...
    def test_a_worker_does_not_offer_service_query(self):
        process, _ = run(DWORKER_MAIN, 'worker', 'service-query', '--help')

        self.assertNotEqual(process.returncode, 0)
        self.assertIn('invalid choice', process.stderr)

    def test_a_foreman_offers_service_query(self):
        process, _ = run(DWORKER_MAIN, 'foreman', 'service-query', '--help')
